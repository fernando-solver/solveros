...
Perguntas que aparecem com frequência sobre o Solverkitty.

---

## "Claude Code já tem memória persistente. Qual a diferença do Solverkitty?"
...
**Memória do Claude é sobre você, a pessoa. Memória do Solverkitty é sobre o seu trabalho.**

### Em uma tabela

| Aspecto | Claude Code memory | Solverkitty memory |
|---|---|---|
...
Solverkitty sabe: *último avanço no capítulo 3 em 2026-05-01, total 1.847 palavras, fonte: `livro/03_capitulo_3/rascunho.md`*.

Você consegue **gerar relatório, calcular ritmo, comparar semanas** com Solverkitty. Não consegue com memória do Claude.

#### 2. Memória passiva vs operação ativa

Memória do Claude é **passiva** — Claude usa **se** lembrar.

Solverkitty é **ativo** — o hook `SessionStart` injeta contexto do trabalho no boot, skills procedurais executam rotinas reais (gerar dashboard, arquivar pasta, indexar PDFs), slash commands transformam dado em entregável. A memória não só "lembra" — ela **opera**.

#### 3. Portabilidade

A memória do Claude mora em `~/.claude/` — vinculada à sua instalação.

A memória do Solverkitty mora no seu workspace (filesystem + SQLite) — você copia a pasta, leva pra outra máquina, e tem tudo de volta. Outro LLM lê os mesmos arquivos.

### A resposta curta e honesta

Os dois juntos são melhores que cada um sozinho. O Solverkitty usa a memória do Claude pra entender o **jeito do usuário**, e o `pmo.db` pra operar o **trabalho do usuário**. Não tem redundância — cada um cobre o que o outro não cobre.

---

## "Posso usar sem Claude Code? Funciona com ChatGPT?"

Os arquivos do Solverkitty (CLAUDE.md, historico.md, objetivos.md, skills/) são puramente texto markdown e Python. Qualquer LLM com acesso ao filesystem consegue ler.
...
A única coisa que sai é o que o Claude Code envia pra API da Anthropic durante a sessão (e isso já é como Claude Code funciona, independente do Solverkitty).
...
**Mentoria/turma é pago.** Pra implementar o Solverkitty com método e acompanhamento — desenhar suas áreas, customizar skills pro seu trabalho, montar dashboards específicos — vou abrir turmas periódicas.
...
- **claude-mem** — resolve memória conversacional (entre sessões). Solverkitty resolve operação ao longo de meses. Camadas complementares.
- **LangChain / CrewAI / AutoGen** — frameworks pra construir agentes em código. Solverkitty é filesystem editorial — não tem código de agente, só convenção.
- **Notion / Obsidian** — sistemas de notas estruturados. Não executam ações, não têm log auditável, não geram entregáveis.

A diferença é tese: **agente vive no seu computador real, não num ambiente isolado**. Solverkitty é a forma mais barata e direta de transformar isso em prática.

---

## Faltou alguma?

Abre issue em https://github.com/fernando-solver/solverkitty/issues
